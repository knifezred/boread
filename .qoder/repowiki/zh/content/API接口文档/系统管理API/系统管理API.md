...
- [sys_setting.go](file://app/server/internal/repository/sys_setting.go)
- [system-manage.ts](file://app/web/src/service/api/system-manage.ts)
- [setup.go](file://app/server/internal/handler/v1/setup.go)
</cite>
...
## 简介
本文件面向boread项目的系统管理API，覆盖用户管理、角色权限、部门管理、菜单配置、字典管理、系统配置管理、系统日志等后台管理能力。文档重点阐述RBAC权限模型实现、数据权限控制、操作审计日志与系统配置管理，并提供完整的CRUD与批量处理接口说明、权限分配流程、系统架构设计、数据一致性保障与安全防护策略，以及管理员操作指南与API集成示例。
...
### 系统配置管理API
**新增** 系统配置管理API提供灵活的配置存储与管理能力，支持按分类组织配置项，提供完整的CRUD操作和批量处理功能。

- 接口概览
  - GET /api/manage/setting/:id
  - POST /api/manage/setting/page
  - POST /api/manage/setting
  - PUT /api/manage/setting/:id
  - DELETE /api/manage/setting/:id
  - GET /api/manage/setting/categories
  - GET /api/manage/setting/by-category/:category
  - POST /api/manage/setting/batch-save

- 关键特性
  - **分类管理**：支持按category组织配置，便于配置的逻辑分组
  - **类型安全**：支持多种valueType（string、number、boolean、json等）
  - **编辑控制**：可配置配置项是否可编辑，系统内置配置不可删除
  - **批量操作**：支持批量保存配置（upsert模式）
  - **状态管理**：支持启用/禁用配置项
  - **权限控制**：受按钮级权限保护，防止未授权访问

```mermaid
sequenceDiagram
participant C as "客户端"
participant H as "SettingHandler"
participant S as "SettingService"
participant R as "SysSettingRepository"
participant DB as "数据库"
C->>H : POST /api/manage/setting
H->>S : Create(req, opID)
S->>R : 检查category+key唯一性
R-->>S : 未重复
S->>R : 插入配置记录
R-->>S : 成功
S-->>H : 配置对象
...
**图表来源**
- [router.go:152-160](file://app/server/internal/router/router.go#L152-L160)
- [setting.go:21-52](file://app/server/internal/service/setting.go#L21-L52)

**章节来源**
- [router.go:152-160](file://app/server/internal/router/router.go#L152-L160)
- [setting.go:21-52](file://app/server/internal/service/setting.go#L21-L52)
- [setting.go:69-80](file://app/server/internal/dto/setting.go#L69-L80)
...
- **配置管理优化**：配置项查询支持按分类缓存，批量保存操作采用事务处理，减少数据库往返次数。
...
- [setting.go:126-150](file://app/server/internal/service/setting.go#L126-L150)

## 故障排查指南
- 认证失败
  - 现象：返回授权头缺失或令牌无效错误。
  - 排查：确认请求头格式为"Bearertoken"，令牌未过期。
- 权限不足
  - 现象：返回按钮code权限拒绝。
...
- 业务约束错误
  ...
  - **配置项错误**：
    - category+key重复：检查配置项唯一性约束
    - 非可编辑配置：尝试修改不可编辑配置项会被拒绝
    - 系统内置配置：不可删除系统标记的配置项
- 日志查询异常
  - 现象：分页查询无结果或字段不匹配。
...
- [setting.go:54-86](file://app/server/internal/service/setting.go#L54-L86)

## 结论
boread系统管理API以清晰的分层架构与严格的中间件控制实现了完善的RBAC权限体系与数据权限边界。通过树形结构的菜单与部门管理、灵活的字典与日志查询、细粒度的按钮级授权，以及新增的系统配置管理能力，满足后台管理的多样化需求。系统配置管理提供了灵活的配置存储方案，支持分类组织、类型安全、批量操作等功能。建议在高并发场景下对按钮权限校验引入缓存优化，并持续完善审计与监控能力。
...
- **系统配置管理**
  - 按业务模块创建配置分类，如"系统设置"、"业务配置"、"第三方集成"等；
  - 使用批量保存接口进行配置项的批量导入和更新；
  - 对于敏感配置项，设置为不可编辑以防止误修改；
  - 定期备份配置数据，确保系统可恢复性。
...
const getConfig = async (id: number) => {
  const response = await fetchGetSettingById(id);
  return response.data;
};

// 批量保存配置
const batchSaveConfigs = async (category: string, items: ConfigItem[]) => {
  const response = await fetchBatchSaveSettings({
    category,
    items
  });
  return response.data;
...
// 按分类获取配置
const getCategoryConfigs = async (category: string) => {
  const response = await fetchGetSettingsByCategory(category);
  return response.data;
};
```

**章节来源**
- [system-manage.ts:478-551](file://app/web/src/service/api/system-manage.ts#L478-L551)