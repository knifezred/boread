...
- **重要规则：响应结构体不得使用 omitempty**
  - 所有 DTO 响应结构体（*Response）和嵌入其中的 Model 的 `json` 标签中**严禁使用 `omitempty`**
  - 原因：`omitempty` 会导致零值字段（0、false、""）在 JSON 中被省略，前端访问到 `undefined` 引发 TypeError
  - 前端应总能收到明确的字段值（`0` / `false` / `null`），而非字段缺失
  - 例外：请求结构体、`binding:"omitempty"`（校验用）、`json:"-"`（忽略字段）不受此约束
- 服务层
