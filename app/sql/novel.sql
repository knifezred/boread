-- =====================================================================
-- Business Schema (v2) —— 小说阅读平台业务表
-- Engine: InnoDB / utf8mb4_unicode_ci / MySQL 8.0+
-- Scope:  阅读者、书库、书架、章节、笔记/划线、评论/书评、分类标签、角色、统计
-- Changes:
--   1. book 表增加书名+作者逻辑唯一约束（仅未删除记录）
--   2. book_chapter 改为按文件存储章节，唯一键 (book_id, file_id, chapter_no)
--   3. reader_read_progress 增加 file_id，唯一键改为 (reader_id, book_id, file_id)
--   4. 新增 book_character 表，支持角色/人物管理
-- =====================================================================
...
--   - 书名+作者 在未删除时唯一（使用虚拟生成列实现条件唯一约束）
--   - primary_file_id 指定默认阅读的文件版本
-- ---------------------------------------------------------------------
...
CREATE TABLE `book` (
  ...
  `update_time`       DATETIME(3)     NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  `deleted_at`        DATETIME(3)     NULL     DEFAULT NULL             COMMENT '软删除时间',
  -- 虚拟列，用于实现未删除时的书名+作者唯一约束
  `uniq_title` VARCHAR(255) GENERATED ALWAYS AS (IF(deleted_at IS NULL, title, NULL)) STORED,
  `uniq_author` VARCHAR(128) GENERATED ALWAYS AS (IF(deleted_at IS NULL, author, NULL)) STORED,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_title_author` (`uniq_title`, `uniq_author`),
  KEY `idx_title` (`title`),
...
-- ---------------------------------------------------------------------
-- Table: book_chapter (按文件存储的章节索引)
--   唯一键 (book_id, file_id, chapter_no)，允许同一本书的不同文件拥有独立章节
-- ---------------------------------------------------------------------
...
CREATE TABLE `reader_read_progress` (
  ...
  `reader_id`       BIGINT UNSIGNED NOT NULL                          COMMENT '阅读者 id',
  `book_id`         BIGINT UNSIGNED NOT NULL                          COMMENT '书籍 id',
  `file_id`         BIGINT UNSIGNED NOT NULL                          COMMENT '当前阅读的文件 id (book_file.id)',
  `chapter_id`      BIGINT UNSIGNED NOT NULL                          COMMENT '当前章节 id (book_chapter.id)',
  ...
  `create_time`     DATETIME(3)     NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_reader_book_file` (`reader_id`, `book_id`, `file_id`),
  KEY `idx_chapter` (`chapter_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='阅读进度表（按文件版本记录）';
...
CREATE TABLE `book_character` (
  ...
  `alias`                   VARCHAR(255)    DEFAULT NULL              COMMENT '别名/称号',
  `description`             TEXT            DEFAULT NULL              COMMENT '角色简介',
  `avatar_url`              VARCHAR(512)    DEFAULT NULL              COMMENT '形象图 URL',
  `first_appear_chapter_id` BIGINT UNSIGNED DEFAULT NULL              COMMENT '首次出场章节 id (book_chapter.id)',
  `sort_order`              INT             NOT NULL DEFAULT 0        COMMENT '排序',
  `create_time`             DATETIME(3)     NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  `update_time`             DATETIME(3)     NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_book_id` (`book_id`),
  KEY `idx_name` (`name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='书籍角色/人物表';
