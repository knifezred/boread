...
onMounted(async () => {
  try {
    const token = await getUgreenToken()
    if (!token) {
      errorMsg.value = '无法获取绿联授权令牌'
      return
    }
  ...
  <div class="flex-col-center gap-20px py-24px">
    ...
    <template v-if="loading">
      <NFlex vertical align="center" justify="center" :size="16">
        <NSpin size="large" />
        <span class="text-14px text-#999">{{ $t('common.ugreenAuthorizing') }}</span>
      </NFlex>
    </template>
    ...
    <template v-else-if="errorMsg">
      ...
      <div class="flex-y-center justify-between gap-12px">
        <NButton class="flex-1" round block type="success" @click="handleSwitchToPwd">
          切换到账号密码登录
        </NButton>
      </div>
    </template>
      ...
      <NCard :bordered="true" size="small" class="w-full">
        <NFlex vertical :size="8">
          <div class="flex items-center gap-10px">
            ...
            <span class="text-16px font-500">{{ profile.userName || profile.userId }}</span>
          </div>
          ...
          <div v-else class="text-12px text-#52c41a mt-4px">
            已有绑定账号
          </div>
        </NFlex>
      </NCard>
