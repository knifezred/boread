...
async function handleSave() {
  ...
  try {
    ...
    if (error) {
      message.error($t('page.setup.saveFailed') + ': ' + (error.message || error));
      return;
    }
    saved.value = true;
...
<template>
  <div class="relative size-full flex-center overflow-hidden bg-#f5f5f5 dark:bg-#1e1e1e">
    <NCard :bordered="false" class="relative z-4 w-auto rd-12px shadow-md">
      <div class="w-420px lt-sm:w-300px">
        <header class="text-center pb-24px">
          ...
          <h3 class="text-24px text-primary font-500 mt-12px">{{ $t('system.title') }}</h3>
        </header>
        <main>
          <template v-if="!saved">
            <h3 class="text-18px text-primary font-medium mb-16px">{{ $t('page.setup.title') }}</h3>
            <p class="text-13px text-gray-500 mb-20px">{{ $t('page.setup.description') }}</p>
            <NForm :model="formValue" size="large" label-placement="top">
              ...
              <NFormItem :label="$t('page.setup.password')" path="password">
                <NInput v-model:value="formValue.password" type="password" show-password-on="click" placeholder="" />
              </NFormItem>
              <NSpace vertical :size="16" class="pt-8px">
                <NButton type="primary" size="large" round block :loading="saving" @click="handleSave">
          ...
          <template v-else>
            <div class="text-center py-20px">
              <div class="i-carbon:checkmark-filled text-48px text-green-500 mx-auto mb-16px" />
              <h3 class="text-20px text-primary font-500 mb-12px">{{ $t('page.setup.successTitle') }}</h3>
              <p class="text-14px text-gray-500 mb-24px">{{ $t('page.setup.successDescription') }}</p>
              <NButton size="large" round type="primary" @click="router.push({ name: 'login' })">
