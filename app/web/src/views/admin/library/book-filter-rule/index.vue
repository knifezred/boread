...
const searchParams = ref<Api.BookManage.FilterRuleSearchParams>({
...
const { columns, columnChecks, data, loading, pagination, getData, getDataByPage } = useNaivePaginatedTable({
  ...
  columns: () => [
    ...
    },
    {
      ...
      render: (row: Api.BookManage.BookContentFilterRule) => (
...
const editingData: Ref<Api.BookManage.BookContentFilterRule | null> = ref(null);

function handleAdd() { operateType.value = "add"; editingData.value = null; openModal(); }
function handleEdit(item: Api.BookManage.BookContentFilterRule) { operateType.value = "edit"; editingData.value = { ...item }; openModal(); }
