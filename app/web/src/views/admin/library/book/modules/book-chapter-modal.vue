...
const searchParams = ref({ current: 1, size: 20, bookId: props.bookId || null, fileId: null, chapterNo: null })
...
const { columns, data, pagination, loading, getDataByPage } = useNaivePaginatedTable({
  ...
  columns: () => [
    ...
    {
      key: "volumeNo", title: "卷号", align: "center",
      render: (row: Api.BookManage.BookChapter) => {
        if (!row.volumeNo) return null
        const volLabel = row.volumeTitle || `卷${row.volumeNo}`
        return h(NTag, { type: "info", size: "small" }, () => volLabel)
      },
    },
...
<template>
  <NModal
    v-model:show="visible" :title="title" preset="card" class="max-w-4xl"
    @update:show="(val) => { if (val) getDataByPage(1) }"
  >
    <template #header-extra>
      <NButton size="small" type="primary" ghost @click="reparseVisible = true">
        {{ $t("page.admin.library.bookChapterRule.form.reParse.title") }}
      </NButton>
    </template>
    <NDataTable
      :columns="columns" :data="data" size="small" :loading="loading" :pagination="pagination"
      :row-key="(row: Record<string, any>) => row.id" remote :flex-height="true" min-height="540"
    />
    <template #footer>
      <NSpace justify="end">
