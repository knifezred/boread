...
const readerBorder = computed(() =>
  darkMode.value ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.04)",
);
...
<template>
  <div
    class="min-h-screen font-sans relative transition-colors duration-300"
  ...
  >
    <main class="flex justify-center bg-[#F4F0E7]">
      <div class="flex gap-4 items-start w-full max-lg:max-w-full max-w-800px">
        ...
        >
          <NSpin :show="contentLoading || bookLoading">
            <template v-if="sortedChapters.length">
              <!-- 书籍详情（仅首个窗口包含第1章时显示） -->
              <div
                v-if="sortedChapters[0].chapterNo === 1 && bookInfo"
                class="mb-10 pb-10 border-b text-center"
              ...
              >
                ...
                <h1 class="lg:text-3xl text-2xl font-bold mb-2 text-inherit">
                  {{ bookInfo.title }}
                </h1>
                <p
                  class="text-sm mb-8"
                ...
                >
                  {{ bookInfo.author }} {{ $t("page.book.reader.writtenBy") }}
                </p>
                <div class="flex justify-center gap-6 lg:gap-16 mb-8">
                  <div class="flex flex-col gap-1">
                    <span class="text-base font-medium">{{
                      bookInfo.categoryName || $t("page.book.reader.unknown")
                    }}</span>
                    <span
                      class="text-xs"
                      :class="darkMode ? 'text-gray-400' : 'text-gray-500'"
                  ...
                  <div class="flex flex-col gap-1">
                    <span class="text-base font-medium">{{
                      bookInfo.serialStatus === "1"
                        ? $t("page.book.reader.ongoing")
                        : $t("page.book.reader.finished")
                    }}</span>
                    <span
                      class="text-xs"
                      :class="darkMode ? 'text-gray-400' : 'text-gray-500'"
                  ...
                  <div class="flex flex-col gap-1">
                    <span class="text-base font-medium">{{
                      formatWordCount(bookInfo.totalWords || 0)
                    }}</span>
                    <span
                      class="text-xs"
                      :class="darkMode ? 'text-gray-400' : 'text-gray-500'"
                ...
                <div
                  v-if="bookInfo.intro"
                  class="mb-6 max-w-2xl lg:mx-18 text-align-left"
                >
                  <p
                    v-for="(paragraph, index) in bookInfo.intro
                      .split('\n')
                      .filter((p) => p.trim())"
                    :key="index"
                ...
                <template v-for="ch in sortedChapters" :key="ch.chapterNo">
                  <div :id="`chapter-${ch.chapterNo}`">
                    <div class="text-center mb-8 lg:mb-10 pt-2">
                      <h1
                        class="lg:text-2xl text-xl font-bold mb-4 text-inherit"
                      >
                        {{ ch.title }}
                      </h1>
                    </div>
                    <div
                      class="mb-12 text-inherit leading-relaxed"
                      :style="{
                    ...
                    >
                      <template
                        v-for="(paragraph, index) in ch.content
                          .split('\n')
                          .filter((p) => p.trim())"
                        :key="index"
                      >
              ...
              <!-- 没有更多章节 -->
              <div
                v-if="!hasNext && sortedChapters.length > 0"
            ...
            <div
              v-else-if="!contentLoading && !bookLoading"
          ...
          <!-- 桌面端底部导航 -->
          <div
            class="flex justify-center gap-6 pt-6 border-t shrink-0 max-lg:hidden"
            :style="{ borderColor: readerBorder }"
          >
            <NButton
              :disabled="!hasPrev"
        ...
        <!-- 桌面端右侧操作栏 -->
        <aside
          class="sticky top-72px w-56px shrink-0 flex flex-col items-center gap-5 pt-6 max-lg:hidden"
        >
          <div
          ...
          <div
            ...
            :title="
              $t(
                darkMode
                  ? 'page.book.reader.dayMode'
                  : 'page.book.reader.nightMode',
              )
            "
            @click="darkMode = !darkMode"
          >
            <SvgIcon
              :icon="darkMode ? 'solar:sun-bold' : 'solar:moon-linear'"
              size="20"
            />
            <span class="text-[10px] leading-none">{{
              darkMode
                ? $t("page.book.reader.dayMode")
                : $t("page.book.reader.nightMode")
            }}</span>
          </div>
          <div
            class="w-40px h-40px rd-1 flex flex-col items-center justify-center gap-0.5 cursor-pointer transition-all duration-200"
      ...
      <!-- 手机端顶部操作栏 -->
      <div
        class="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-2 lg:hidden"
        :style="{
          backgroundColor: readerBg,
      ...
      >
        ...
        <div
          class="text-sm font-medium truncate max-w-[60%] text-center text-inherit"
        >
          {{
            sortedChapters.length > 0
              ? sortedChapters[1]?.title || sortedChapters[0].title
              : ""
          }}
        </div>
        <div
      ...
      <CatalogModal
        ...
        :chapter-no="
          sortedChapters.length > 0
            ? sortedChapters[1]?.chapterNo || sortedChapters[0].chapterNo
            : 1
        "
        :dark-mode="darkMode"
        @select="goToChapter"
