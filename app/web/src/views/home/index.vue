...
const { options: serialStatusOptions, labelMap: serialStatusLabelMap } =
...
const filterConfig = computed(() => ({
  ...
  serialStatus: [
    { label: $t("page.book.home.all"), value: "" },
    ...serialStatusOptions.value.map((opt) => ({
  ...
  wordCount: [
    { label: $t("page.book.home.all"), value: "" },
    ...wordCountOptions.value.map((opt) => ({
      label: opt.label as string,
      value: opt.value as string,
    })),
  ],
  tags:
    tagOptions.value.length > 1
      ? tagOptions.value
      : [{ label: $t("page.book.home.all"), value: "" }],
  updateTime: [
    { label: $t("page.book.home.all"), value: "" },
...
async function loadBooks(page = 1) {
  ...
  const { updateTimeFrom, updateTimeTo } = parseUpdateTimeRange(
...
<template>
  <div class="flex gap-4 overflow-hidden lt-sm:flex-col h-full bg-layout">
    <!-- ============ 左侧筛选栏 ============ -->
    <aside
      class="w-240px shrink-0 lt-sm:w-full lt-sm:max-h-60 lt-sm:overflow-y-auto"
    >
    ...
    <div class="flex-1 flex flex-col overflow-hidden min-w-0">
      ...
      <div class="card-wrapper p-4 bg-container mb-4">
        <div class="flex items-center gap-3 flex-wrap">
          ...
          <NButton round @click="router.push('/book-reader')">
            {{
              $t("page.book.home.importBooks")
            }}
          </NButton>
        </div>
      </div>
      ...
      <!-- 排序栏 -->
      <div
        class="card-wrapper px-4 py-3 bg-container mb-4 flex items-center justify-between flex-wrap gap-2"
      >
      ...
      <div class="flex-1 overflow-y-auto">
        <NSpin :show="loading">
          ...
          <div
            ...
            class="flex flex-col items-center justify-center py-30"
          >
            <div class="mb-5 opacity-60">
              <SvgIcon local-icon="empty-file" />
            </div>
            <p class="text-16px text-gray-400">
              {{ $t("page.book.home.noContent") }}
            </p>
          </div>
            ...
            >
              ...
              <div class="flex-1 flex flex-col gap-2 min-w-12">
                ...
                <div class="flex items-center gap-2 text-gray-400 text-xs">
                  ...
                  <span>{{
                    book.categoryName || $t("page.book.home.uncategorized")
                  }}</span>
                  <span class="text-gray-400 mx-1">|</span>
                  <span>{{ serialStatusLabelMap[book.serialStatus] }}</span>
                </div>
                <p
                  class="flex-1 text-sm text-gray-500 m-0 leading-6 line-clamp-3 whitespace-pre-line"
                >
                  {{ book.intro || $t("page.book.home.noIntro") }}
                </p>
                <div
                  class="flex items-center gap-2 justify-left text-xs text-gray-400"
                >
